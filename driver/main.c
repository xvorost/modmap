...
NTSTATUS Main() {
	...
	*(PVOID*)&MiAllocateVad = addr;
	...
	if(!addr)
	{
		...
		addr = FindPatternImage(base, "\x48\x89\x6C\x24\x00\x48\x89\x74\x24\x00\x57\x41\x56\x41\x57\x48\x83\xEC\x40\x44\x0F\xB6\x79\x00\x33\xC0\x44\x0F\xB6\x71\x00\x48\x8B\xEA", "xxxx?xxxx?xxxxxxxxxxxxx?xxxxxx?xxx");
	}
	if (!addr) 
	{
	...
	*(PVOID*)&MiInsertVadCharges = addr;
	...
	{
		// Windows 11 signature
		addr = FindPatternImage(base, "\x48\x89\x5C\x24\x00\x48\x89\x6C\x24\x00\x48\x89\x74\x24\x00\x48\x89\x7C\x24\x00\x41\x54\x41\x56\x41\x57\x48\x83\xEC\x20\x8B\x41\x1C", "xxxx?xxxx?xxxx?xxxx?xxxxxxxxxxxxx");
	}
	if (!addr) 
	{
